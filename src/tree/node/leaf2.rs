...
use crate::mmap::{Page, ReadOnlyPage, Reader, Writer};
...
impl<'a> Leaf<'a> {
    /// Initializes an Mmap-backed COW B+ tree root as an empty leaf node.
    pub fn init_tree_root(writer: Writer) -> usize {
    ...
    pub fn read<'r>(reader: &'r Reader, page_num: usize) -> Result<Leaf<'r>> {
        let page = reader.read_page(page_num)?;
        let node_type = header::get_node_type(page.as_ref())?;
    ...
    pub fn insert<'w>(&self, writer: &'w Writer, key: &[u8], val: &[u8]) -> Result<LeafEffect<'w>> {
    ...
    pub fn update<'w>(&self, writer: &'w Writer, key: &[u8], val: &[u8]) -> Result<LeafEffect<'w>> {
        if key.len() > consts::MAX_KEY_SIZE {
            return Err(NodeError::MaxKeySize(key.len()));
        }
        if val.len() > consts::MAX_VALUE_SIZE {
    ...
    fn insert_iter<'l>(&'l self, key: &'l [u8], val: &'l [u8]) -> InsertIterator<'l, 'a> {
        InsertIterator {
    ...
    fn update_iter<'l>(&'l self, key: &'l [u8], val: &'l [u8]) -> UpdateIterator<'l, 'a> {
        UpdateIterator {
            leaf_itr: self.iter().peekable(),
            key,
            val,
...
mod tests {
    ...
    fn new_test_store() -> (Store, usize) {
        ...
        let root_ptr = Leaf::init_tree_root(store.writer());
        (store, root_ptr)
    }
    ...
    fn test_insert_max_key_size() {
        ...
        let result = Leaf::read(&reader, root_ptr)
            .unwrap()
            .insert(&writer, key, "val".as_bytes());
    ...
    fn test_insert_max_value_size() {
        ...
        let result = Leaf::read(&reader, root_ptr)
            .unwrap()
            .insert(&writer, "key".as_bytes(), val);
        assert!(
    ...
    fn test_find_none() {
        ...
        let leaf = Leaf::read(&reader, root_ptr).unwrap();
    ...
    fn test_update_max_key_size() {
        ...
        let result = Leaf::read(&reader, root_ptr)
            .unwrap()
            .update(&writer, key, "val".as_bytes());
        assert!(matches!(result, Err(NodeError::MaxKeySize(x)) if x == consts::MAX_KEY_SIZE + 1));
    }
    ...
    fn test_update_non_existent() {
        ...
        let result = Leaf::read(&reader, root_ptr).unwrap().update(
            &writer,
            "key".as_bytes(),
            "val".as_bytes(),
        );
